...
const Groups = () => {
  ...
  const filteredGroups = groups.filter((group) => {
    ...
    return groupName.includes(searchTerm.toLowerCase()) || teacherName.includes(searchTerm.toLowerCase());
  });
                    ...
                    <td className="py-5 px-6 hidden md:table-cell">
                      <div className="space-y-1">
                        <div className="flex items-center space-x-2 text-[9px] font-black text-slate-500 uppercase">
                          <FiCalendar />
                          <span>{group.start_date ? new Date(group.start_date).toLocaleDateString() : '20.04.2025'}</span>
                        </div>
                        <div className="flex items-center space-x-2 text-[9px] font-black text-slate-400 uppercase italic">
                          <span>To: {group.end_date ? new Date(group.end_date).toLocaleDateString() : '12.05.2025'}</span>
                        </div>
                      </div>
                    </td>
                    <td className="py-5 px-6 text-center">
