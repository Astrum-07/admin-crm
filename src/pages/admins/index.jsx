...
const Admins = () => {
  ...
  const { data: admins = [], isPending, isError } = useQuery({
    queryKey: ['admins'],
    queryFn: async () => {
      const res = await axios.get(`${baseUrl}/api/staff/all-admins`, {
  ...
  if (isPending) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] space-y-4">
        ...
        <p className="font-black uppercase tracking-[3px] text-xs text-slate-400">Adminlar yuklanmoqda...</p>
      </div>
    );
  }
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <p className="text-slate-400 font-bold text-sm tracking-wide">
            Tizimning barcha ma'murlari nazorati
          </p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-black text-white uppercase text-[11px] tracking-[2px] font-black">
                <th className="py-5 px-6">Ism Familiya</th>
                  ...
                  <tr key={admin._id} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="py-5 px-6">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={`font-black uppercase text-[10px] tracking-widest ${admin.status === 'faol' ? 'text-black' : 'text-red-600'}`}>
                          {admin.status}
                        </span>
                      </div>
                    </td>
